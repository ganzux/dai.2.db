...
RECORDAR CAMBIAR LO DE PROVISIONAL DEL MD5 Y DE LA HOJA CSS

http://www.alvarito.net/provisional/hoja.css
../alumnos/Alvaro/css/hoja.css

http://www.alvarito.net/provisional/md5.js
../alumnos/Alvaro/js/md5.js


FALTA:
------------------------------------------------------------------ Crear la Cookie Temporal
- Ver el contenido del texto y restringir caracteres
- Fecha del último mensaje
- Upload y Dwnload de iconos
- Buscar
- Lista de usuarios
- ¿paginar? por id de 50 en 50
...
--           @'C:\Documents and Settings\AlVaRiTo\Escritorio\foro modificado el lunes\cuerpo.sql'
...
<HTML>
	...
	<HEAD>
		...
		<LINK href="http://www.alvarito.net/provisional/hoja.css" rel="stylesheet" type="text/css">
...
BEGIN
  ...
  IF id_tema_pasada IS NOT NULL THEN
	...
	<center><form name="nuevo_mensaje" method="GET" action="foro.publicar" onSubmit="return CompruebaMensaje()">
    <p>Asunto
...
BEGIN
  ...
  IF texto IS NOT NULL AND tema IS NOT NULL THEN
	...
	IF asunto IS NULL THEN
		INSERT INTO foro_mensajes VALUES (seq_foro_men.NEXTVAL,SYSDATE,'(Sin Asunto)',texto,id_usuario_captada,tema);
...
HTP.P('<script src="http://www.alvarito.net/provisional/md5.js" type="text/javascript"></script>
<form name="datos">
...
IF galleta.num_vals!=0 THEN
  ...
  IF chorro IS NULL THEN
    ...
    <script src="http://www.alvarito.net/provisional/md5.js" type="text/javascript"></script>');
...
nomb_usua := busca_usuario;
	...
	FOR REG IN (SELECT foro_temas.nombre_tema, foro_mensajes.asunto, foro_mensajes.contenido, foro_mensajes.fecha
	...
	ELSE
		cabecera('Error');
		HTP.P('<H2><center>Error, no está logueado</center></H2>');
