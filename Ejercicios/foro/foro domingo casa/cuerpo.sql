...
SET DEFINE OFF
SET LINESIZE 6000
--           @'C:\Documents and Settings\AlVaRiTo\Escritorio\foro domingo casa\cuerpo.sql'
		...
		ELSE
			...
			IF id_cookie = id_autor_mensaje THEN
				...
				IF asunt IS NOT NULL THEN
					UPDATE foro_mensajes SET asunto = asunt, contenido = texto WHERE id = mensaje;
				-- Si no es nulo, lo publicamos con el asunto
				ELSE
					UPDATE foro_mensajes SET asunto = '(Sin Asunto)', contenido = texto WHERE id = mensaje;
				END IF;
			ELSE
