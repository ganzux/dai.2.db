...
- Archivos en los mensajes de usuario
- Iconos mejores
- Lista de usuarios
- Buscar
*/
  ...
  IF id_tema_pasada IS NOT NULL THEN
    ...
    LOOP
	  ...
	  IF REG.archivo IS NOT NULL THEN
		HTP.P('<div align="center"><A HREF="docs/'||REG.archivo||'"><img src="http://www.gifmania.com/rotulos/download/download.gif" alt="Bajar Archivo '||REG.archivo||'" border=0></A></div>');
	  ELSE
		HTP.P('<div align="center"><img src="../alumnos/Alvaro/jpg/no.bmp" alt="No hay ningún documento asociado" border=0></div>');
	  END IF;
	  HTP.P('</TD>');
...
BEGIN
  ...
  IF id_tema_pasada IS NOT NULL THEN
	...
	<center><form name="nuevo_mensaje" method="POST" action="foro.publicar" onSubmit="return CompruebaMensaje()" encType=multipart/form-data>
	...
	if (document.nuevo_mensaje.texto.value == "")
		...
		{
		document.nuevo_mensaje.archivo.value=archivo.document.nuevo_mensaje.archivo.value
		archivo.close()
		return true
		}
    }
