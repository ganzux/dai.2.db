--           @'C:\Documents and Settings\AlVaRiTo\Escritorio\foro domingo casa\Código Fuente - Code\cuerpo.sql'
...
HTP.P('	<!-- Cabecera de la WeB -->
	...
	<!-- Imagen en cabecera -->
	<div align="center"><A HREF="http://www.alvarito.net"><img src="../alumnos/Alvaro/jpg/webito.gif" width="150" height="75" border=0></A></div>
...
BEGIN
	...
	-- También almacenaré el número de id del usuario por si pudiese editar el mensaje
	usu := busca_usuario;
	IF usu IS NOT NULL THEN
		SELECT tipo_usuario INTO tip FROM foro_usuarios WHERE usu = nick;
		SELECT id INTO num_usu FROM foro_usuarios WHERE usu = nick;
	END IF;
  ...
  IF id_tema_pasada IS NOT NULL THEN
    ...
    LOOP
	  ...
	  -- Si el usuario Actual es quien escribió el mensaje, le damos la posibilidad de editarlo
	  IF num_usu = REG.id_usuario THEN
		HTP.P('<TD><center><A HREF="foro.edita?mensaje='||REG.id||'"><img src="../alumnos/Alvaro/jpg/edit.gif" alt="Editar mensaje" border=0></A>'||REG.contenido||'</TD>');
	  
	  -- Si no es el autor, no lo edita
	  ELSE
	...
	<table width="75%"  border="0" align="center" cellspacing="0">
      ...
      IF busca_usuario IS NOT NULL THEN
            HTP.P('<A HREF="foro.nuevo_mensaje?id_tema_pasada='||id_tema_pasada||'"><img src="../alumnos/Alvaro/jpg/reply.gif" border=0 alt="Escribir Nuevo Mensaje"></A>');
      END IF;
      HTP.P('</div></td>
	    <td width="5%"></td>
	    <td width="30%">');
	...
	</table><br><center><H5>Total: '||total||' Mensajes en el tema</h5></center>');
    pie;
  ELSE
    cabecera('Error');
...
nom VARCHAR2(35);
texto_real VARCHAR2(4050);
asunt_real VARCHAR2(100);
BEGIN
  ...
  IF texto IS NOT NULL AND tema IS NOT NULL THEN
	...
	-- Substituyo la < y la >
	select replace(texto,'<','&lt;') INTO texto_real from dual;
	select replace(texto_real,'>','&gt;')INTO  texto_real from dual;
	...
	IF asunto IS NULL THEN
		INSERT INTO foro_mensajes VALUES (seq_foro_men.NEXTVAL,SYSDATE,'(Sin Asunto)',texto_real,id_usuario_captada,tema,archivo);
	-- Si no es nulo, lo publicamos con el asunto
	ELSE
		select replace(asunto,'>','&gt;')INTO asunt_real from dual;
		select replace(asunt_real,'<','&lt;')INTO asunt_real from dual;
		INSERT INTO foro_mensajes VALUES (seq_foro_men.NEXTVAL,SYSDATE,asunt_real,texto_real,id_usuario_captada,tema,archivo);
	END IF;
...
BEGIN
  ...
  ELSE
    cabecera('Error');
    HTP.P('<H3><center>No está conectado al sistema</center></H3>
	<center><b>Pulse <A HREF="foro.login">AQUÍ</A> para Iniciar Su sesión</b></center>');
    pie;
  END IF;
END salir;
...
num_mens NUMBER(6);
CURSOR tmas IS SELECT * FROM vista_temas ORDER BY id DESC;
total NUMBER(6):=0;
BEGIN
	...
	HTP.P('</TABLE><H5><center>Total: '||total||' Temas</center></H5>');
	pie;
END muestra_temas;
...
login_real VARCHAR2(100);
mail_real VARCHAR2(100);

BEGIN
-- Reemplzamos < y >

select replace(nombre,'<','&lt;') INTO nombre_real FROM dual;
select replace(nombre_real,'>','&gt;') INTO nombre_real FROM dual;
select replace(login,'<','&lt;') INTO login_real FROM dual;
select replace(login_real,'>','&gt;') INTO login_real FROM dual;
select replace(mail,'<','&lt;') INTO mail_real FROM dual;
select replace(mail_real,'>','&gt;') INTO mail_real FROM dual;



-- Miramos si existe ese login
SELECT COUNT(*) INTO contador_logi FROM foro_usuarios WHERE foro_usuarios.nick = ''||login_real||'';
-- Miramos si existe el mail
SELECT COUNT(*) INTO contador_mail FROM foro_usuarios WHERE foro_usuarios.email = ''||mail_real||'';

-- Si no existe lo damos de alta
IF contador_logi = 0 AND contador_mail = 0 THEN
	INSERT INTO foro_usuarios VALUES (seq_foro_usu.NEXTVAL,nombre_real,login_real,pass,mail_real,0,SYSDATE);
	COMMIT;
  cabecera('Usuario Creado');
	HTP.P('<H3><center>Su usuario ha sido dado de alta</center></H3>
	<b><center>Pulse <A HREF="foro.login">AQUÍ</A> para Iniciar Su sesión</center></b>');
  pie;
...
id_usuario NUMBER(5);
asunto_real VARCHAR2(100);
BEGIN
  -- Si existen el Usuario Y el Asunto
  IF asunto IS NOT NULL AND usuario IS NOT NULL THEN
  
	select replace(asunto,'<','&lt;') INTO asunto_real FROM dual;
	select replace(asunto_real,'>','&gt;') INTO asunto_real FROM dual;

    SELECT foro_usuarios.id INTO id_usuario FROM foro_usuarios WHERE usuario = foro_usuarios.nick;
    INSERT INTO foro_temas VALUES (seq_foro_tem.NEXTVAL,id_usuario,asunto_real);
...
nomb_usua := busca_usuario;
	...
	HTP.P('</table></center><H5><center>Total: '||total||' Mensajes</center></h5>');
			pie;
...
<tr>
  ...
  <tr>
		<td class="row1" align="left" valign="top"><span class="postbody"><b>Breve resumen</b></span><br />
    <span class="postbody">El foro es un proyecto que será presentado el día 5 de febrero de 2006; cumpliendo los requisitos mínimos de escribir mensajes y loguearse, he implementado un sistema de subidas de archivos, de gestión de uuarios mediante Cookies, de envío de correos electrónicos, un sistema de moderación, con dos rangos de usuario...</span></td>

  </tr>  <tr>
		<td class="row1" align="left" valign="top"><span class="postbody"><b>Futuro próximo</b></span><br />
    <span class="postbody">La idea futura es integrar un sistema de envío de mensajes privados entre usuarios, la posibilidad de que todo uuario pueda editar sus mensajes y que los moderadores puedan editarlos todos e integrar algún JavaScript de validación de e-mail.</span></td>
...
nomb_edit VARCHAR2(80);
mail_edit VARCHAR2(80);
...
nick_usua := busca_usuario;
	...
	-- Editamos los < y >
	select replace(nom,'<','&lt;') INTO nomb_edit FROM dual;
	select replace(nomb_edit,'>','&gt;') INTO nomb_edit FROM dual;
	select replace(mai,'<','&lt;') INTO mail_edit FROM dual;
	select replace(mail_edit,'>','&gt;') INTO mail_edit FROM dual;
	...
	IF nom IS NOT NULL AND nom != nomb_usua THEN
		UPDATE foro_usuarios SET nombre = nomb_edit WHERE nick = nick_usua;
		COMMIT;
	...
	IF mai IS NOT NULL AND mai != mail_usua AND numero = 0 THEN
		UPDATE foro_usuarios SET email = mail_edit WHERE nick = nick_usua;
		COMMIT;
		HTP.P('<center><H4>El campo <b>e-mail</b> ha sido modificado a "<b>'||mail_edit||'</b>"</h4></center>');
	ELSIF numero != 0 THEN
		HTP.P('<center><H4>La dirección de e-mail "<b>'||mail_edit||'</b>" ya existe.</h4></center>');
	END IF;
...
IF nik IS NOT NULL THEN
	...
	END IF;
	  ...
	  HTP.P('<td align="center">'||REG.id||'</td>
	    ...
	    HTP.P('<td align="center">'||REG.fecha_registro||'</td>');
	  ...
	  total := total + 1;
	  END LOOP;
	HTP.P('</table><center><H4>Nota: Los usuarios con fondo <font color="lightblue">AZUL</font> son moderadores.</h4></center>
	<br><center><H5>Total: '||total||' Usuarios</h5></center>');
	...
	cabecera('Envío de Correo electrónico');
		...
		IF (operacion = 'Recordar') THEN
			...
			IF num_usu != 0 THEN
				...
				SELECT dbms_random.string('a',10) INTO pass FROM dual;
...
FOR contador IN 1..coll_resp.count LOOP
  ...
  htp.p('  <tr>
    ...
    <td><img src="../alumnos/Alvaro/jpg/ok.gif" border=0></td>
  </tr>');
END LOOP;
commit;
HTP.P('</TABLE>
...
BEGIN
	...
	IF clave IS NOT NULL OR autor IS NOT NULL THEN
		...
		END LOOP;
		END IF;
		HTP.P('</table><center><H5>Total: '||total||' Resultados</h5></center>');
...
PROCEDURE edita (mensaje NUMBER DEFAULT NULL) IS
usuario VARCHAR2(35);
id_cookie NUMBER(7);
id_autor_mensaje NUMBER(5);
numero NUMBER(3);
asunto_ant VARCHAR2(60);
contenido_ant VARCHAR2(4000);
BEGIN
	cabecera('Edición de mensaje','edita_mensaje.asunt');
	usuario := busca_usuario;
	
	-- Primero miramos si está o no logueado
	IF usuario IS NULL THEN
		HTP.P('Debe estar usted logueado para poder editar mensajes');
	-- Si está logueado, miramos si ha pasado un parámetro correcto
	ELSE
		IF mensaje IS NULL THEN
			HTP.P('Error, no se especificó el mensaje a editar');
		-- Está logueado y ha especificado mensaje
		ELSE
			--Ahora miraremos si existe el mensaje
			SELECT COUNT(*) INTO numero FROM foro_mensajes WHERE id = mensaje;
			IF numero = 0 THEN
				HTP.P('Error, no existe ese mensaje');
			-- Existe el mensaje, está logueado y ha pasado un mensaje
			ELSE
				-- Cargamos la id de la cookie
				SELECT id INTO id_cookie FROM foro_usuarios WHERE nick = usuario;
				-- Cargamos la id del autor del mensaje
				SELECT id_usuario INTO id_autor_mensaje FROM foro_mensajes WHERE id = mensaje;
				-- Si no coinciden, damos el error
				IF id_cookie != id_autor_mensaje THEN
					HTP.P('Error, usted no es el autor del mensaje');
				ELSE
					-- Reuperamos los daots del antiguo mensaje
					SELECT asunto, contenido INTO asunto_ant, contenido_ant FROM foro_mensajes WHERE id = mensaje;
					-- y los escribimos
					HTP.P('<center><h2>Edición de mensaje</h2><form name="edita_mensaje" method="GET" action="foro.edita_mensaje" onSubmit="return CompruebaMensaje()" encType=multipart/form-data>
    <p>Asunto <input name="asunt" type="text" size="50" maxlength="50" value="'||asunto_ant||'"></p>
    <p>Mensaje <textarea name="texto" cols="90" rows="6">'||contenido_ant||'</textarea></p>
	<input type ="hidden" name="mensaje" value="'||mensaje||'">
    <p><input type="submit" value="Editar Mensaje">
    <input type="reset" value="Borrar"></p>
    </form></center>
    ...
    function CompruebaMensaje()
    {
	if (document.edita_mensaje.texto.value == "")
		{
		alert("No ha escrito ningún mensaje")
		document.edita_mensaje.texto.focus()
		return false
		}
...
-- falta poner bonito y comprobar que el mensaje exista o no

PROCEDURE edita_mensaje (asunt VARCHAR2 DEFAULT NULL, texto VARCHAR2 DEFAULT NULL, mensaje VARCHAR2 DEFAULT NULL) IS
usuario VARCHAR2(35);
id_cookie NUMBER(7);
id_autor_mensaje NUMBER(5);

asunto_real VARCHAR2(126);
texto_real VARCHAR2(4050);

BEGIN

-- Reemplazamos los < y >
	select replace(asunt,'<','&lt;') INTO asunto_real FROM dual;
	select replace(asunto_real,'>','&gt;') INTO asunto_real FROM dual;
	select replace(texto,'<','&lt;') INTO texto_real FROM dual;
	select replace(texto_real,'>','&gt;') INTO texto_real FROM dual;
	
	cabecera('Editando Mensaje');
	htP.P('Editando Mensaje');
	-- Lo primero es si el usuario está logueado o no
	usuario := busca_usuario;	
	IF usuario IS NULL THEN
		HTP.P('Error, ha de estar conectado para poder editar');
	-- Usuario conectado
	ELSE
		--Ahora miraremos si existen el texto y el mensaje
		IF texto IS NULL OR mensaje IS NULL THEN
			HTP.P('Error, la id de mensaje y el texto son Obligatorios');
		ELSE
			-- Comprobamos que el mensaje coincida con la id del usuario logueado
			-- Cargamos la id de la cookie
			SELECT id INTO id_cookie FROM foro_usuarios WHERE nick = usuario;
			-- Cargamos la id del autor del mensaje
			SELECT id_usuario INTO id_autor_mensaje FROM foro_mensajes WHERE id = mensaje;
			...
			IF id_cookie = id_autor_mensaje THEN
				HTP.P('Editando mensaje');
				IF asunt IS NOT NULL THEN
					UPDATE foro_mensajes SET asunto = asunto_real, contenido = texto_real WHERE id = mensaje;
				-- Si no es nulo, lo publicamos con el asunto
				ELSE
					UPDATE foro_mensajes SET asunto = '(Sin Asunto)', contenido = texto_real WHERE id = mensaje;
				END IF;
			ELSE
				HTP.P('Error, sólo el Autor puede editar su mensaje');
			END IF;
