...
<table width="850" height="400" cellspacing="10" border="0">
	...
	while ($row = mysql_fetch_row($busqueda)) {
		...
		while($row2=mysql_fetch_row($busqueda2)){
			if($valor%4==0){
				echo "<td><table align='center' border='2' width='230'><tr><td><b><a href='salas.php?id_cine=".$row2[0]."'><font size='2'>".$row2[2]."</a></td></b></tr>" .
					"<tr><td><b>DIR :</b>".$row2[3]."<br><b>TLF :</b>".$row2[4]."</td></tr>" .
					"</table></td></tr>";
			}
			else{
				echo "<td><table align='center' border='2' width='230'><tr><td><b><a href='salas.php?id_cine=".$row2[0]."'><font size='2'>".$row2[2]."</a></td></b></tr>" .
