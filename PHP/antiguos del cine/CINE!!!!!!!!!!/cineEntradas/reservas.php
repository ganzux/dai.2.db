<?	
	...
	$delete="Delete from reservas_temporal where fecha<(Select TIME_TO_SEC(NOW())-12000)";
	...
	if(isset($_POST['butacas'])){
		...
		for($i=1;$i<=$butacas;$i++){
			...
			if(isset($_POST["asiento".$i])&&$_POST["asiento".$i]==1){
				...
				$temp="select r.id_proyeccion,b.butaca from reservas_temporal r,butacas_temporal b where r.id=b.id_reserva and r.id_proyeccion=".$id_proy." and b.butaca=".$i;
				
				$temp2="select r.id_proyeccion,b.butaca from reservas r,butacas b where r.id=b.id_reserva and r.id_proyeccion=".$id_proy." and b.butaca=".$i;
...
if($valor2==0){
	...
	if(!isset($_GET['valor'])){
		$butacas_temp="Insert into reservas_temporal(id_proyeccion,fecha) values(".$id_proy.",TIME_TO_SEC(TIME(NOW())))";
		
		mysql_query($butacas_temp)or die ('Error en reservas_temp');
		$select="select max(id) from reservas_temporal";
		$select2=mysql_query($select);
		$select=mysql_fetch_row($select2);
		$_SESSION['reserva_temporal']=$select[0];
		
		for($i=0;$i<count($array);$i++){
			$insert="Insert into butacas_temporal(id_reserva,butaca) values(".$select[0].",".$array[$i].")";
